...
##################################################
#  ====== TASK 1: READ PREDICTION (REVERTED) ====#
...
##################################################
#  ====== TASK 2: CATEGORY PREDICTION (ENSEMBLE) #
#  (V9 logic, but training on ALL 200,000 samples) #
##################################################
...
def preprocess_text_pipeline(text, nlp_tools):
    ...
    text = stemming(text, nlp_tools['stemmer'])
    return text
...
def extract_numeric_features(review_dict):
    ...
    Numeric metadata features:
      - rating
      - log(1 + n_votes), safely handling missing/negative values
    """
...
def train_category_model(catTrain, nlp_tools):
    """
    Train an ensemble of Logistic Regression models on the FULL dataset.
    """
    print("  Running NLTK-based preprocessing on training data...")
    preprocessed_reviews = [preprocess_text_pipeline(d['review_text'], nlp_tools)
                            for d in catTrain]
    y_all = [d['genreID'] for d in catTrain]
    numeric_all = np.array([extract_numeric_features(d) for d in catTrain])

    # --- THIS IS THE V10 FIX ---
    # The 10k subsampling block has been REMOVED.
    # We now train on the full dataset.
    print(f"  Using all {len(catTrain)} samples for category training...")
    X_text = preprocessed_reviews
    y_train = y_all
    ...
    print("  Fitting word-level TfidfVectorizer (1-2 grams, ~30k features)...")
    ...
    print("  Fitting char-level TfidfVectorizer (3-5 char ngrams, word-boundary, ~30k features)...")
    ...
    print("  Building feature matrices for ensemble models...")
    X_word_num = hstack([X_word, numeric_sparse])      # word + numeric
    X_char_num = hstack([X_char, numeric_sparse])      # char + numeric
    X_full = hstack([X_word, X_char, numeric_sparse])  # word + char + numeric

    print("  Training word+numeric Logistic Regression...")
    model_word = LogisticRegression(
        C=10,
        max_iter=1000,
        solver='liblinear'
    )
    model_word.fit(X_word_num, y_train)

    print("  Training char+numeric Logistic Regression...")
    model_char = LogisticRegression(
        C=10,
        max_iter=1000,
        solver='liblinear'
    )
    model_char.fit(X_char_num, y_train)

    print("  Training full (word+char+numeric) Logistic Regression...")
    model_full = LogisticRegression(
        C=10,
        max_iter=1000,
        solver='liblinear'
    )
    model_full.fit(X_full, y_train)

    # Package models and vectorizers into bundles
    model_bundle = {
        "word": model_word,
        "char": model_char,
        "full": model_full
    ...
    print("  Category ensemble training complete.")
    return model_bundle, aux_bundle

def _align_proba(base_classes, model, proba):
    """
    Align model.predict_proba output to base_classes order.
    """
    aligned = np.zeros((proba.shape[0], len(base_classes)))
...
def predict_categories(pairs, catTest, model_bundle, aux_bundle, nlp_tools):
    """
    Generates category predictions using the ensemble.
    """
    print("  Running NLTK-based preprocessing on test data...")
    ...
    numeric_scaler = aux_bundle["numeric_scaler"]
    numeric_scaled = numeric_scaler.transform(numeric_array)
    numeric_sparse = csr_matrix(numeric_scaled)

    tfidf_word = aux_bundle["tfidf_word"]
    tfidf_char = aux_bundle["tfidf_char"]

    print("  Transforming test data with TF-IDF (word and char)...")
    X_word_test = tfidf_word.transform(texts)
    X_char_test = tfidf_char.transform(texts)

    X_word_num_test = hstack([X_word_test, numeric_sparse])
    X_char_num_test = hstack([X_char_test, numeric_sparse])
    X_full_test = hstack([X_word_test, X_char_test, numeric_sparse])

    model_word = model_bundle["word"]
    model_char = model_bundle["char"]
    model_full = model_bundle["full"]

    print("  Getting class probabilities from ensemble models...")
    proba_full_raw = model_full.predict_proba(X_full_test)
    base_classes = model_full.classes_  # reference ordering

    proba_word_raw = model_word.predict_proba(X_word_num_test)
    proba_char_raw = model_char.predict_proba(X_char_num_test)

    proba_full = _align_proba(base_classes, model_full, proba_full_raw)
    proba_word = _align_proba(base_classes, model_word, proba_word_raw)
    proba_char = _align_proba(base_classes, model_char, proba_char_raw)

    # Weighted ensemble: full has highest weight
    w_full = 0.6
    w_word = 0.2
    w_char = 0.2
    proba_ensemble = (
    ...
    print("  Selecting predicted genres from ensemble probabilities...")
    best_class_idx = np.argmax(proba_ensemble, axis=1)
    best_labels = [base_classes[i] for i in best_class_idx]
...
if __name__ == "__main__":
    ...
    # This will now train on the full 200,000 samples
    category_model_bundle, aux_bundle = train_category_model(catTrain, nlp_tools)
    category_predictions = predict_categories(
        pairsCategory, catTest, category_model_bundle, aux_bundle, nlp_tools
    )
